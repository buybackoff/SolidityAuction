 ...
 * TokenMock API
 */
export class TokenMock extends SoltsiceContract {
    public static get artifacts() { return require('../artifacts/TokenMock.json'); }

    public static get bytecodeHash() {
        // we need this before ctor, but artifacts are static and we cannot pass it to the base class, so need to generate
        let artifacts = TokenMock.artifacts;
    ...
    public static async new(deploymentParams: W3.TX.TxParams, ctorParams?: {_name: string, _symbol: string, _decimals: BigNumber | number}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<TokenMock> {
        w3 = w3 || W3.default;
        if (!privateKey) {
            let contract = new TokenMock(deploymentParams, ctorParams, w3, link);
            await contract._instancePromise;
            return contract;
        } else {
            let data = TokenMock.newData(ctorParams, w3);
            let txHash = await w3.sendSignedTransaction(W3.zeroAddress, privateKey, data, deploymentParams);
            let txReceipt = await w3.waitTransactionReceipt(txHash);
            let rawAddress = txReceipt.contractAddress;
            let contract = await TokenMock.at(rawAddress, w3);
            return contract;
        }
    }

    public static async at(address: string | object, w3?: W3): Promise<TokenMock> {
        let contract = new TokenMock(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    public static async deployed(w3?: W3): Promise<TokenMock> {
        let contract = new TokenMock('', undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }
    ...
    public static newData(ctorParams?: {_name: string, _symbol: string, _decimals: BigNumber | number}, w3?: W3): string {
        // tslint:disable-next-line:max-line-length
        let data = SoltsiceContract.newDataImpl(w3, TokenMock.artifacts, ctorParams ? [ctorParams!._name, ctorParams!._symbol, ctorParams!._decimals] : []);
        return data;
    }
