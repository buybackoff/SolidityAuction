...
export class Auction extends SoltsiceContract {
    ...
    public static async New(deploymentParams: W3.TX.TxParams, ctorParams?: {_owner: string, _wallet: string}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<Auction> {
        w3 = w3 || W3.Default;
        if (!privateKey) {
    ...
    public static NewData(ctorParams?: {_owner: string, _wallet: string}, w3?: W3): string {
        // tslint:disable-next-line:max-line-length
        let data = SoltsiceContract.NewDataImpl(w3, Auction.Artifacts, ctorParams ? [ctorParams!._owner, ctorParams!._wallet] : []);
        return data;
    }
