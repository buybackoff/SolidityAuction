 ...
 * AuctionHub API
 */
export class AuctionHub extends SoltsiceContract {
    public static get artifacts() { return require('../artifacts/AuctionHub.json'); }

    public static get bytecodeHash() {
        // we need this before ctor, but artifacts are static and we cannot pass it to the base class, so need to generate
        let artifacts = AuctionHub.artifacts;
    ...
    public static async new(deploymentParams: W3.TX.TxParams, ctorParams?: {_wallet: string, _tokens: string[], _rates: BigNumber[] | number[], _decimals: BigNumber[] | number[]}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<AuctionHub> {
        w3 = w3 || W3.default;
        if (!privateKey) {
            let contract = new AuctionHub(deploymentParams, ctorParams, w3, link);
            await contract._instancePromise;
            return contract;
        } else {
            let data = AuctionHub.newData(ctorParams, w3);
            let txHash = await w3.sendSignedTransaction(W3.zeroAddress, privateKey, data, deploymentParams);
            let txReceipt = await w3.waitTransactionReceipt(txHash);
            let rawAddress = txReceipt.contractAddress;
            let contract = await AuctionHub.at(rawAddress, w3);
            return contract;
        }
    }

    public static async at(address: string | object, w3?: W3): Promise<AuctionHub> {
        let contract = new AuctionHub(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    public static async deployed(w3?: W3): Promise<AuctionHub> {
        let contract = new AuctionHub('', undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }
    ...
    public static newData(ctorParams?: {_wallet: string, _tokens: string[], _rates: BigNumber[] | number[], _decimals: BigNumber[] | number[]}, w3?: W3): string {
        // tslint:disable-next-line:max-line-length
        let data = SoltsiceContract.newDataImpl(w3, AuctionHub.artifacts, ctorParams ? [ctorParams!._wallet, ctorParams!._tokens, ctorParams!._rates, ctorParams!._decimals] : []);
        return data;
    }
