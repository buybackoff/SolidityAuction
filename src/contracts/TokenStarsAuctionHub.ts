...
export class TokenStarsAuctionHub extends SoltsiceContract {
    ...
    public static async New(deploymentParams: W3.TX.TxParams, ctorParams?: {_wallet: string}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<TokenStarsAuctionHub> {
        w3 = w3 || W3.Default;
        if (!privateKey) {
    ...
    public static NewData(ctorParams?: {_wallet: string}, w3?: W3): string {
        // tslint:disable-next-line:max-line-length
        let data = SoltsiceContract.NewDataImpl(w3, TokenStarsAuctionHub.Artifacts, ctorParams ? [ctorParams!._wallet] : []);
        return data;
    }
